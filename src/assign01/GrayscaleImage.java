...
public class GrayscaleImage {
    ...
    public double getPixel(int x, int y) {
        if(x>this.imageData[0].length|| y> this.imageData.length)
            throw new IllegalArgumentException("X and or Y are out of bounds");
        var brightness = imageData[y][x];
        return brightness;
    }
     ...
     * @return true if the images are the exact same
     */
    @Override
    public boolean equals(Object other) {
        ...
        if (otherImage.imageData[0].length != this.imageData[0].length) {
            return false;
        } else if (this.imageData.length != otherImage.imageData.length) {
            return false;
        }
    ...
    public double averageBrightness() {
        var totalElements = this.imageData.length * this.imageData[0].length;
        var avg = 0.0;
        var total = 0.0;
    ...
    public GrayscaleImage normalized() {
        var average = this.averageBrightness();
        var normalizedGrayscaleImage = new GrayscaleImage(this.imageData);
        var multiplyingFactor = 127.0 / average;

        for (int i = 0; i < this.imageData.length; i++) {
            for (int j = 0; j < this.imageData[0].length; j++) {
                normalizedGrayscaleImage.imageData[i][j] = imageData[i][j] * multiplyingFactor;
            }
        }
        return normalizedGrayscaleImage;
    }
    ...
    public GrayscaleImage mirrored() {
        var mirroredImage = new GrayscaleImage(this.imageData);
        for (int i = 0; i < this.imageData.length; i++) {
            for (int j = 0; j < this.imageData[0].length; j++) {
                mirroredImage.imageData[i][j] = this.imageData[i][imageData[0].length - j - 1];
            }
        }
     ...
     *
     * @param startRow starting at zero
     * @param startCol starting at zero
     * @param width
     * @param height
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height) throws IllegalArgumentException {
        if (this.imageData.length < startRow + height || this.imageData[0].length < startCol + width|| startRow<0||startCol<0)
            throw new IllegalArgumentException("inputs are outside the image");

        var croppedData = new double[height][width];
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                croppedData[i][j] = this.imageData[startRow + i][startCol + j];
            }
        }
        var croppedImage = new GrayscaleImage(croppedData);
        return croppedImage;
    }
    ...
    public GrayscaleImage squarified() {
        ...
        int difference;
        double[][] squarifiedData;
        if (height == width) {
            return this;
        } else if (height < width) {
            difference = width - height;
            var removalAmount = difference / 2;
            squarifiedData = new double[height][height];
            for (int i = 0; i < squarifiedData.length; i++) {
                for (int j = 0; j < squarifiedData.length; j++) {
                    squarifiedData[i][j] = this.imageData[removalAmount + i][j];
                }
            }
        }else{
                difference = height - width;
                var removalAmount = difference / 2;
                squarifiedData = new double[width][width];
                for (int i = 0; i < squarifiedData.length; i++) {
                    for (int j = 0; j < squarifiedData.length; j++) {
                        squarifiedData[i][j] = this.imageData[i][j + removalAmount];
                    }
                }
            }


            var squarifiedImage = new GrayscaleImage(squarifiedData);
